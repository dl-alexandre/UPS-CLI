...
```
go-cli-template/
├── cmd/ups/              # Entry point (renamed by setup)
│   └── main.go
├── internal/
│   ├── api/                      # HTTP client
│   │   ├── client.go
...
- `ups` - Application name (lowercase, single word)
- `UPS-CLI` - Repository name
- `CLI for UPS APIs (Tracking, Rating, Shipping)` - Project description
