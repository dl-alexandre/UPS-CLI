...
func main() {
	...
	cliver.BinaryName = "ups"

	var c cli.CLI
	ctx := kong.Parse(&c,
		kong.Name("ups"),
		kong.Description("CLI for UPS APIs (Tracking, Rating, Shipping)"),
		kong.UsageOnError(),
	...
	go func() {
		...
		updateCache := cache.New(cache.DefaultDir("ups"), 24*time.Hour)
