...
```bash
gh repo create ups --public --source=. --push
```
...
1. **Create the tap repository**:
   ```bash
   gh repo create homebrew-tap --public --description "Homebrew tap for ups"
   ```
...
2. **Test installation**:
   ```bash
   # Direct download
   curl -L -o ups https://github.com/dl-alexandre/ups/releases/latest/download/ups-linux-amd64
   chmod +x ups
   ./ups version
...
1. Visit: `https://pkg.go.dev/github.com/dl-alexandre/ups`
2. Click **"Request indexing"**
3. This allows `go install github.com/dl-alexandre/ups/cmd/ups@latest` to work
...
**Problem**: Some files still contain `dl-alexandre` or `ups`
